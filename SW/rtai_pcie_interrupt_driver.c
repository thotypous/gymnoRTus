...
#define AVALONTOP_WDATA 0x00
#define AVALONTOP_WADDR 0x04
#define AVALONTOP_RDATA 0x08

// Periodic task period in ns
#define PERIOD 200000

#define DMA_BITS 32
#define DMA_SIZE (1024*sizeof(uint64_t))
...
static int irq_handler(unsigned irq, void *cookie_) {
    ...
    const uint32_t flag = ioread32(avalontop_base + AVALONTOP_RDATA);
    ...
    if (likely(irq_is_ours)) {
        ...
        for(i = 0; i < 1024; i++, data++) {
            if(dma_ptr[i] != data) {
                rt_printk("pcie_interrupt_driver: error at i=%d, should be %llu, but was %llu\n",
                    i, data, dma_ptr[i]);
                break;
            }
        }
...
void thread_main(long thread) {
    while (1) {
        ...
        iowrite32(epoch, avalontop_base + AVALONTOP_WDATA);
...
static void pci_remove(struct pci_dev *dev) {
    ...
    if(dma_ptr != NULL) {
        rt_printk("pcie_interrupt_driver: liberando memoria, mas antes: %llx\n", dma_ptr[0]);
